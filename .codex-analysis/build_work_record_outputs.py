...
ROOT = Path(__file__).resolve().parents[1] / "통합업무기록"
...
EVENTS_BY_MONTH = {
    ...
    (2026, 8): {
    5: [("지역 6 목록·좌표 보완", "green"),
        ("MapWebApp 6.1 반영", "green")],
    10: [("측정결과보고서·자료 정리", "green"),
         ("분석 보고서 통합", "blue")],
    11: [("충전소 목록·지도 서비스", "green"),
         ("현장 운영정보·지도 서비스", "orange")],
    19: [("3·11·14kW 후보 교체", "green"),
         ("목록·지도·보고서 6.3 검증", "blue")],
    20: [("8주차 고조파 백분율 변환", "green")],
    },
    (2026, 9): {
    1: [("측정결과 고조파 백분율 변환", "green"),
        ("6~9지역 전원 품질 분석", "blue")],
    2: [("재생에너지 거리 분석 기록 보완", "blue")],
    8: [("6~9지역 ICCU 검증 분석·PPT", "blue")],
    9: [("여수·광양 왜곡 인자 분석·PPT", "blue")],
    10: [("6~10지역 계측 분석·PPT 갱신", "blue")],
    },
}
...
<div class='history'>
  ...
  <article class='history-card wide month-filter-item'
      data-months='2026-08'>
    <div class='version'>Total_list · MapWebApp 6.1 <span>8/5</span></div>
    <h3>지역 6 충전소 목록과 지도 데이터를 함께 보완</h3>
    <ul>
      <li>정선군·태백시 후보가 기존 목록과 지도에 충분히 반영되지 않아 지역 6 현장 관리 범위가 달라질 수 있었던 문제를 점검.</li>
      <li>중복 후보를 제외한 71개소를 Total_list에 추가하고, 정선군 40개소와 태백시 31개소의 좌표를 함께 보완.</li>
      <li>일련번호 06-262부터 06-332까지를 연결하고, 후보·일련번호 중복 및 수식 오류가 없는지 검사.</li>
      <li>보완된 지역 6 목록을 MapWebApp 6.1에 반영해 목록과 지도 데이터의 같은 관리 기준을 유지.</li>
    </ul>
  </article>
  <article class='history-card month-filter-item'
      data-months='2026-08'>
    <div class='version'>측정결과보고서 · 데이터 정리 <span>8/10</span></div>
    <h3>누락되던 충전소 정보와 측정자료를 한 기준으로 정리</h3>
    <ul>
      <li>06-253 이후 충전소를 선택해도 위치와 업체명이 자동으로 표시되지 않아 현장 결과를 확인하기 어려웠음.</li>
      <li>자동 입력 대상을 06-332까지 복구하고, 1~6지역 고조파 값을 같은 기준의 백분율로 통일했으며 2,390개 측정자료를 12개 지역별로 정리.</li>
      <li>평창·정선 80곳 분석을 하나의 보고서로 묶어 후속 검토가 쉬워졌음. 백분율 변환본은 별도 결과물이며 실제 Excel 열기 확인은 남아 있음.</li>
    </ul>
  </article>
  <article class='history-card month-filter-item'
      data-months='2026-08'>
    <div class='version'>충전소 목록 · 지도 서비스 <span>8/11</span></div>
    <h3>실제 조사 대상에 맞춰 116곳을 목록·지도·보고서에 일괄 반영</h3>
    <ul>
      <li>7·12지역 후보에 급속 충전기가 섞여 목록·지도·측정결과보고서마다 조사 대상이 다르게 보일 우려가 있었음.</li>
      <li>7지역 71곳과 12지역 45곳, 총 116곳을 실제 조사 대상인 완속 후보로 교체하고 세 자료에 같은 정보를 반영.</li>
      <li>공개 지도에서 새 대상 검색과 기존 방문기록 사용이 가능해져 현장 준비와 결과 확인을 같은 기준으로 진행할 수 있음.</li>
    </ul>
  </article>
  <article class='history-card wide month-filter-item'
      data-months='2026-08'>
    <div class='version'>현장 운영정보 · 지도 서비스 <span>8/11</span></div>
    <h3>현장 상태를 번호와 색상으로 구분해 관리 대상 명확화</h3>
    <ul>
      <li>측정완료·충전불가·미방문 상태가 목록과 지도에서 한눈에 구분되지 않아 현장 우선순위를 판단하기 어려웠음.</li>
      <li>완료 대상은 001~080, 충전불가 대상은 081 이후로 정리하고 충전불가 314곳은 지도에서 빨간색으로 표시했으며, 미방문 544곳은 운영 대상에서 제외.</li>
      <li>현장 진행상황을 즉시 구분할 수 있는 별도 검토본을 마련. 3지역은 자료 부족으로 보류했고 MapWebApp 6.3은 아직 운영 서버에 배포하지 않음.</li>
    </ul>
  </article>
</div>"""
        ...
        },
        {
            ...
            "phases": [
                ("2026-07",
                 ...
                 "수준과 크게 불일치.",
                 "계측 터미널 문제를 확인한 뒤 (구) 계측 터미널과 "
                 "별도로 받은 (신) 계측 터미널을 구성하고, 각 "
                 "터미널을 해당 계측기에 연결해 약 30A의 정상값을 확인.")
            ],
        },
        {
            "no": "07",
...
DAILY_IMPROVEMENTS = [
    ...
    ("2026-07-20", "최신 Total_list를 지도·보고서에 동기화", [
        "문제: 4~12지역 2순위 후보 보강 후 최신 목록과 기존 지도·측정결과보고서 간 충전소 수·번호 불일치.",
    ...
    ("2026-07-28", "MapWebApp 6.0 운전자 촬영 연동 구성 · 계측 터미널 점검", [
        ...
        "문제: 7kW 완속충전기 계측 중 파워 어널라이저 전류값이 약 2.3A로 표시되어, 220V 조건에서 예상되는 약 30A 수준과 크게 불일치.",
        "조치: 계측 터미널 문제를 확인한 뒤 (구) 계측 터미널과 별도로 받은 (신) 계측 터미널을 구성하고, 각 터미널을 해당 계측기에 연결.",
    ...
    ("2026-08-05", "지역 6 목록·좌표 보완 및 MapWebApp 6.1 반영", [
        "문제: 정선군·태백시 후보가 기존 Total_list와 지도에 충분히 반영되지 않아 지역 6 현장 관리 범위가 달라질 수 있었음.",
        "조치: 중복 후보 69개소를 제외하고 정선군 40개소, 태백시 31개소 등 71개소를 Total_list에 추가.",
        "조치: 추가 71개소의 좌표를 확인하고 일련번호 06-262부터 06-332까지 연결한 뒤, 후보·일련번호 중복과 수식 오류를 검사.",
        "조치: 보완된 지역 6 충전소 목록을 MapWebApp 6.1에 반영.",
        "결과: Total_list와 지도 데이터의 지역 6 충전소 관리 기준을 보완. 실제 운영 화면 확인은 추가 검증 필요.",
    ]),
    ("2026-08-10", "측정결과보고서·자료 정리", [
        "문제: 06-253 이후 충전소를 선택해도 위치와 업체명이 자동으로 표시되지 않아 현장 결과를 확인하기 어려웠음.",
        "개선: 자동 입력 대상을 06-332까지 복구하고, 1~6지역 고조파 값을 같은 기준의 백분율로 통일했으며 2,390개 측정자료를 12개 지역별로 정리.",
        "결과: 평창·정선 80곳 분석을 하나의 보고서로 묶어 후속 검토가 쉬워졌음. 백분율 변환본은 별도 결과물이며 실제 Excel 열기 확인은 남아 있음.",
    ]),
    ("2026-08-11", "충전소 목록·지도 서비스와 현장 운영정보 정비", [
        "문제: 7·12지역 후보에 급속 충전기가 섞여 자료마다 조사 대상이 다르게 보일 우려가 있었고, 현장 상태도 한눈에 구분하기 어려웠음.",
        "개선: 7지역 71곳과 12지역 45곳, 총 116곳을 실제 조사 대상인 완속 후보로 교체해 충전소 목록·지도·측정결과보고서에 같은 정보를 반영.",
        "개선: 측정완료·충전불가·미방문 상태를 번호와 지도 색상으로 구분하고 충전불가 314곳은 빨간색으로 표시했으며 미방문 544곳은 운영 대상에서 제외.",
        "결과: 공개 지도에서 대상 검색과 기존 방문기록 사용이 가능해져 현장 준비와 결과 확인을 같은 기준으로 진행할 수 있음. 3지역은 자료 부족으로 보류했고 MapWebApp 6.3은 별도 검토본으로 운영 서버 배포 전임.",
    ]),
    ("2026-08-19", "3·11·14kW 후보 교체 및 목록·지도·보고서 6.3 검증", [
        "문제: 9·10·11지역의 기존 3·11·14kW 충전소 중 일부가 운영기관·충전기 타입 또는 동시충전 조건과 맞지 않아, 같은 용량의 인접한 다른 물리 충전소로 교체할 필요가 있었음.",
        "조치: 9지역 4건, 10지역 2건, 11지역 5건 등 총 11건만 같은 kW 후보로 교체하고, 번호·우선순위·그룹 필드와 교체 외 4,487개 충전소를 보존.",
        "조치: 12지역의 3kW 후보는 기존 분포 경계 3.920km 밖인 5.509km로 확인돼 기존 행을 유지.",
        "조치: MapWebApp 6.2와 측정결과보고서 8주차에 같은 11건을 반영하고, 지도·CSV 왕복·12개 측정 시트·수식·Excel 읽기 전용 열림을 독립 검증.",
        "결과: 정본과 지도 11건 330개 필드의 차이 0건, CSV 4,498건 왕복 및 보고서의 11건 반영·4,487개 조회행 보존을 확인.",
    ]),
    ("2026-08-20", "8주차 측정결과보고서 고조파 백분율 변환", [
        "문제: 8주차 측정결과보고서의 고조파 값을 기본파 대비 백분율로 일관되게 관리할 별도 결과본이 필요했음.",
        "조치: 실제 데이터가 있는 1~7번 시트의 고조파 55,800개 셀을 변환하고, 비율·수식·서식·비대상 셀·계산 설정을 대조.",
        "결과: 정적 구조 검증을 통과한 별도 백분율 변환본을 생성. 실제 Excel 열기 검증은 수행하지 않음.",
    ]),
    ("2026-09-01", "측정결과 백분율 변환 및 6~9지역 전원 품질 분석", [
        "문제: 2026-08-21 측정결과보고서의 고조파 값을 같은 기준으로 비교하고, 6~9지역 320개 충전지점의 전원 품질을 점검할 필요가 있었음.",
        "개선: 실제 데이터가 있는 1~9번 시트의 고조파 71,800개 셀을 백분율로 변환한 별도 결과본을 만들고, 비율·수식·서식·비대상 셀·계산 설정을 대조.",
        "개선: 6~9지역 320개 지점의 전압, 주파수, 전압·전류 THD와 고조파를 분석해 평창·정선, 신안·무안·영광, 포항, 여수·광양 권역별 확인 항목을 정리.",
        "개선: 6~7지역 160개 지점은 재생에너지 발전설비 위치와 연결해 거리 분석 보고서를 만들었고, 8~9지역은 해당 공간자료가 없어 거리 분석 대상에서 제외.",
        "결과: 전압과 역률은 전반적으로 양호했으며 주파수 특이값 3건과 전류 THD 10% 이상 15건을 재측정 우선 대상으로 구분. 실제 Excel 열기와 주파수 특이값의 현장 재측정은 미완료임.",
        "결과: 현재 자료에서는 재생에너지 설비와 전원 품질 사이의 유의한 관계를 확정하지 않았음. 8~9지역 공간자료와 현장 시계열·PCC·부하 정보가 추가로 필요함.",
    ]),
    ("2026-09-02", "재생에너지 거리 분석 근거 기록 보완", [
        "조치: 9월 1일 6~7지역 160개 지점의 재생에너지 설비 거리 분석 결과와 8~9지역 제외 사유를 업무일지·통합업무기록에 보완.",
        "결과: 2026-09-02 17:03 KST에 업무일지와 공개 HTML 보완 커밋(085252a)을 생성. 거리와 전원 품질의 관계는 공간자료·시계열·PCC·부하 정보 부족으로 확정하지 않았습니다.",
    ]),
    ("2026-09-08", "6~9지역 ICCU 검증 분석 및 발표자료 작성", [
        "조치: 6~9지역 현장 계측 320건을 재분석하고, 전압·전류 THD·역률을 포함한 원천 수치 35,520개를 대조해 분석 자료에 반영.",
        "조치: 지역별 전원 품질 특성, ICCU 입력조건, 대표 시험 매트릭스와 악조건 후보 10개를 담은 발표자료를 작성.",
        "결과: 주파수 의심 3건은 계통 이상으로 확정하지 않고 대표 시험 후보에서 제외했으며, ICCU 영향은 관측값 기반의 검증 대상으로만 기록. 실제 PowerPoint 앱 실행 검증은 독립 확인하지 않았습니다.",
    ]),
    ("2026-09-09", "여수·광양 전원 왜곡 인자 분석 및 51쪽 보고서 작성", [
        "조치: 여수·광양 80건의 전압 THD 평균 3.212%, 중앙값 2.402%, P90 6.933%를 재확인하고, 돌산읍 동측 6개 지점의 전압 H3 집중을 별도 지점군으로 구분.",
        "조치: 산업단지 5 km 이내 38개 지점(전류 THD 6.62%, H11 4.57%)과 비교군 282개 지점(4.93%, 2.84%)을 비교하고, 재생에너지 인접 15개 지점에서는 왜곡 상승 패턴이 뚜렷하지 않음을 정리.",
        "조치: HMC 전동화시스템시험3팀 통화 녹음을 전사해 THD·고조파·11·13차·지역·시간대 등 검토 인자를 정리.",
        "결과: 주파수 이탈 3건을 분석에서 제외하지 않고 유지한 51쪽 `완속충전_계통전원왜곡_인자검증보고서_v16_20260909.pptx`를 작성. 51쪽 렌더링·주요 슬라이드 육안·OOXML 구조 검사를 통과했으며, 특정 제품·운영사·차량 OBC·피더 원인은 확정하지 않았습니다.",
    ]),
    ("2026-09-10", "6~10지역 계측 분석 범위 확장 및 PPT 작성 진행", [
        "조치: 창원까지 측정결과보고서에서 6~10지역을 권역별 80건씩 추출해 분석 기준을 320건에서 400건으로 확장.",
        "결과: 10지역 창원 80건의 전압 THD 평균 2.661%, 전류 THD 평균 5.187%를 원본값으로 집계.",
        "진행: 기존 계통전원 왜곡 인자검증 PPT를 v25까지 수정했고, 10지역을 포함한 분석·보고서 반영을 계속 진행 중입니다. 6~10지역 통합 PPT 확정본과 네이티브 PowerPoint 앱 검증은 아직 완료하지 않았습니다.",
    ]),
]
